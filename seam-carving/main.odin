...
main :: proc() {
	...
	for !rl.WindowShouldClose() {
		...
		if rl.IsWindowResized() {
			state.actual_size.x = u32(rl.GetScreenWidth() - image.width)
			state.actual_size.y = u32(rl.GetScreenHeight())

			state.diff = state.last_size - state.actual_size
			...
			for _ in 0 ..< state.diff.x {
				...
				for j in 1 ..< state.last_size.y {
					...
					for i in last_index +
						...
						2 ..= last_index + int(state.last_size.x) + 2 {
						actual := c[i][0]
						if actual < min {
							min = actual
							min_index = int(i)
